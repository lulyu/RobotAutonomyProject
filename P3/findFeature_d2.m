function [feature, R_his, t_his] = findFeature_d2(NUM_OF_STEP, STARTING_FRAME, K, P_left, P_right)
%% --------------- Starting point ----------------------
filepath_left = 'D:\Study\Robot Autonomy\Project\P3\cmu_16662_p3\NSHLevel2_Images\left';
filepath_right = 'D:\Study\Robot Autonomy\Project\P3\cmu_16662_p3\NSHLevel2_Images\right';
...
pts_next_left = detectHarrisFeatures(img_next_left_gray,'FilterSize',3);
if size(pts_next_left,1)<15
    pts_next_left = detectHarrisFeatures(img_next_left_gray,'FilterSize',3,'MinQuality',0.001);
end
pts_next_right = detectHarrisFeatures(img_next_right_gray,'FilterSize',3);
if size(pts_next_right,1)<15
    pts_next_right = detectHarrisFeatures(img_next_right_gray,'FilterSize',3,'MinQuality',0.001);
end
...
[indexPairs_next_lr,matchmetric_next_lr] = matchFeatures(features_next_left,features_next_right,'MatchThreshold',5,'MaxRatio',0.6);
...
for pic_num=(STARTING_FRAME+1):STARTING_FRAME+NUM_OF_STEP-1
    ...
    pic_num_next = num2str(pic_num_next,'%04i');
    ...
    for i=1:size(indexPairs_ll,1)
        ...
        if (~isempty(pair1) && ~isempty(pair2))
            ...
            if ~isempty(main_features)
                [feature_id,~] = matchFeatures(features_left(ind1,:),single(main_features),'MatchThreshold',1,'MaxRatio',0.6);
            end
            if isempty(feature_id) % new main_feature
                num_of_main_features = num_of_main_features + 1;
                feature{num_of_main_features} = FeaturePoint(num_of_main_features, pic_num-1, pts_left.Location(ind1,:), pts_right.Location(indexPairs_lr(pair1,2),:), features_left(ind1,:));
                % showMatchedFeatures(img_left,img_right,pts_left(indexPairs_lr(pair1,1),:),pts_right(indexPairs_lr(pair1,2),:));
                % showMatchedFeatures(img_next_left,img_next_right,pts_next_left(indexPairs_next_lr(:,1),:),pts_next_right(indexPairs_next_lr(:,2),:));
                % showMatchedFeatures(img_left,img_right,pts_left(indexPairs_lr(:,1),:),pts_right(indexPairs_lr(:,2),:));
                main_features(num_of_main_features,:) = features_left(ind1,:);
            else
                feature{feature_id(2)} = feature{feature_id(2)}.addImageDetected(pic_num-1, pts_left.Location(ind1,:), pts_right.Location(indexPairs_lr(pair1,2),:));
            end
        end
        if (~isempty(pair1))
