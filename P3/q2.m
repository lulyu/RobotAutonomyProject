...
sigma_init_b = noiseModel.Isotropic.Sigmas([ 0.500; 0.500; 0.500; 5.00e-02; 5.00e-02; 5.00e-02 ]);
% ??
...
%       currentBiasKey, currentSummarizedMeasurement, G, w_coriolis));
    ...
    odometry = Pose3(Rot3(ZYXtoE(imu_orientation_step(:,size(imu_orientation_step,2))')), Point3(imu_position_step(:,size(imu_position_step,2)))); % create a measurement for both factors (the same in this case)
...
%     initialEstimate.insert(currentVelKey, LieVector(imu_velocity(:,measurementIndex)));
%     initialEstimate.insert(currentBiasKey, currentBias);
...
for landmarkIndex = 1:size(landmark,2)
    ...
    coord = [landmark_i.coord_3d_world(1) landmark_i.coord_3d_world(3) landmark_i.coord_3d_world(2)]';
